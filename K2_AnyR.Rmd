...
f <- function(index, parents, database) {
    ...
    rowcol <- array(dim = nrow(database))
    ...
    for (i in 1:nrow(database)) {
        ...
        rowcol[i] <- paste(as.character(index.j),as.character(index.k), sep=".")
    }
    print(rowcol)
    # Create Dataframe with Row, Column and Value
    createN <- function(v){
        nonzero <- unique(v)
        val     <- apply(nonzero,1,function(a) sum(v==a))
        row.pos <- apply(nonzero,1,function(a) as.integer(a))
        col.pos <- apply(nonzero,1,function(a) as.integer(sapply(strsplit(a, "\\."), "[", 2)))
                         
        N <- data.frame('Row'=row.pos, 'Col'=col.pos, 'Value'=val)
        return(N)
    }
    N <- createN(rowcol) 
    print(N)           
    #Compute N_{ij}
    Nj_star <- aggregate(.~Row,data=N,sum)[,c(1,3)]
    Nj <- array(0, dim = qi)
    Nj[Nj_star$Row] <- Nj_star$Value
    
    #Compute logN - already row summed
    N$Value <- factorials[N$Value+1]  
    logN_star <- aggregate(.~Row,data=N,sum)[,c(1,3)]
    logN <- array(0, dim = qi)
    logN[logN_star$Row] <- logN_star$Value
...
K2 <- function(database, u, ordering) {
    ...
    for (i in 1:n) {
        ...
        while (OKToProceed & length(pi.i) < u[i]) {
            p.new <- 0
            p.new.index <- NA
            
            #Compute possible parents of i
            i.pos <- which(ordering == i) #Position of i in the ordering
            if (i.pos == ordering[1]) {   #Generalization for different ordering
                break
            } else {
